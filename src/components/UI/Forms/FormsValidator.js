...
export const rules = computed(() => ({
    name: {
        ...
        maxLength: helpers.withMessage('Максимальное количество символов - 20',maxLength(20)),
        minLength: helpers.withMessage('Минимальное количество символов - 5',minLength(5)),
      },
    number:{
        required:helpers.withMessage('Обязательно для заполнения',required),
        maxLength: helpers.withMessage('Введите корректный номер',maxLength(14)),
        regexp: helpers.withMessage('Введите корректный номер',validNumber),
